...
const AuthForm: React.FC<AuthFormProps> = ({
  ...
  const handleSubmit = () => {
    console.log('hello');
    if (
      (mode !== 'passwordReset' && !email) ||
      ((mode === 'register' || mode === 'login') && !password) ||
      (mode === 'register' && !username) ||
      (mode === 'register' && !place) ||
      (mode === 'passwordReset' && !password) ||
      (mode === 'passwordReset' && !confirmPassword)
    ) {
      console.log('hellob');
      ...
      <View style={styles.screen}>
        <View
          ...
          {mode === 'register' || mode === 'login' ? (
            <View style={{marginRight: 16, alignSelf: 'flex-start'}}>
              <NEAHeart />
            </View>
          ) : (
            <></>

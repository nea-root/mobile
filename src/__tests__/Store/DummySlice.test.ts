import reducer, { changeNames, initialState, DummyState } from '@/Store/Slices/DummySlice';
...
describe('DummySlice reducer', () => {
  ...
  it('changeNames updates both firstName and lastName', () => {
    const state = reducer(initialState, changeNames({ firstName: 'Alice', lastName: 'Smith' }));
    expect(state.firstName).toBe('Alice');
    expect(state.lastName).toBe('Smith');
  });

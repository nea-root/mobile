...
describe('cognitoErrorHandler', () => {
  it('returns unknown error message when error is null', () => {
    expect(cognitoErrorHandler(null)).toBe('An unknown error occurred. Please try again.');
  });

  it('returns unknown error message when error has no code', () => {
    expect(cognitoErrorHandler({})).toBe('An unknown error occurred. Please try again.');
    expect(cognitoErrorHandler({ message: 'oops' })).toBe('An unknown error occurred. Please try again.');
  });

  it('maps NotAuthorizedException correctly', () => {
    expect(cognitoErrorHandler({ code: 'NotAuthorizedException' })).toBe('Incorrect username or password.');
  });
  ...
  it('returns unknown error when error is undefined', () => {
    expect(cognitoErrorHandler(undefined)).toBe('An unknown error occurred. Please try again.');
  });
});

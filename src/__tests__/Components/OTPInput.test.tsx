...
describe('OTPInput', () => {
  ...
  it('renders custom length input boxes', () => {
    const {UNSAFE_getAllByType} = render(
      <OTPInput length={4} onSubmit={jest.fn()} />,
    );
    const {TextInput} = require('react-native');
  ...
  it('shows Face ID switch in register mode', () => {
    const {getByText} = render(
      <OTPInput onSubmit={jest.fn()} mode="register" />,
    );
  ...
  it('does not show Face ID switch in reset mode', () => {
    const {queryByText} = render(
      <OTPInput onSubmit={jest.fn()} mode="reset" />,
    );
  ...
  it('calls handleReset when Resend Code is pressed', () => {
    ...
    const {getByText} = render(
      <OTPInput onSubmit={jest.fn()} handleReset={handleReset} />,
    );

...
describe('NeaButton', () => {
  ...
  it('does not call onPress when disabled', () => {
    ...
    const {getByText} = render(
      <NeaButton title="Disabled" onPress={onPress} disabled />,
    );
  ...
  it('applies custom style prop', () => {
    ...
    const {getByText} = render(
      <NeaButton title="Styled" style={customStyle} />,
    );
  ...
  it('applies custom textStyle prop', () => {
    ...
    const {getByText} = render(
      <NeaButton title="Styled text" textStyle={textStyle} />,
    );
  ...
  it('truncates title with numberOfLines=1', () => {
    const {getByText} = render(
      <NeaButton title="A very long button title that should be truncated" />,
    );
    expect(
      getByText('A very long button title that should be truncated'),
    ).toBeTruthy();
  });
});

...
jest.mock('@/Hooks', () => ({
  ...
  useTheme: () => ({
    Colors: { Palette: { white: '#fff' }, alertTextColor: '#000', alertShadowColor: 'rgba(0,0,0,0.3)', alertDividerColor: '#eee' },
    Layout: { fill: { flex: 1 }, center: {}, alignSelfStretch: {}, row: {}, justifyContentSpaceEvenly: {} },
    Gutters: { largeHMargin: {} },
    Fonts: { textBoldRegular: {}, textSmallBold: {}, textRegular: {} },
    MetricsSizes: { large: 24 },
    ViewSizes: {},
    Dimens: {
      modalBorderRaidus: 12, zero: 0, alertShadowOffsetHeight: 2, alertShadowOpacity: 0.25,
      alertShadowRadius: 4, alertShadowElevation: 5, alertMarginTop: 24, alertDescriptionMarginTop: 8,
      alertDividerMarginTop: 16, alertDividerHeight: 1, alertActionHeight: 48, alertActionTextPadding: 4,
      controlBackgroundBlurAmount: 2, controlBackgroundBlurRadius: 2,
    },
  }),
}));
...
const TestConsumer = () => {
  ...
  return (
    <>
      <TouchableOpacity testID="show-btn" onPress={() => ctx.showAlert?.(true, 'T', 'D', [])}>
        <Text>Show</Text>
      </TouchableOpacity>
      <TouchableOpacity testID="hide-btn" onPress={() => ctx.hideModal?.()}>
        <Text>Hide</Text>
      </TouchableOpacity>
      <TouchableOpacity testID="priority-btn" onPress={() => ctx.showAlert?.(true, 'T', 'D', [], true)}>
        <Text>Priority</Text>
      </TouchableOpacity>
    </>
  );
...
describe('AlertModalProviderContext', () => {
  ...
  it('renders without crashing', () => {
    expect(() =>
      render(<AlertModalProviderContext><Text>child</Text></AlertModalProviderContext>)
    ).not.toThrow();
  });

  it('renders children', () => {
    const { getByText } = render(
      <AlertModalProviderContext><Text>inner content</Text></AlertModalProviderContext>
    );
  ...
  it('does not render BlurView when isShow is false', () => {
    const { queryByTestId } = render(
      <AlertModalProviderContext><Text>child</Text></AlertModalProviderContext>
    );
  ...
  it('provides showAlert via context and calls it', () => {
    const { getByTestId } = render(
      <AlertModalProviderContext><TestConsumer /></AlertModalProviderContext>
    );

...
import {forgotPassword} from '@/Services/Authentication/AuthService';
  ...
  const resetPassword = async () => {
    try {
      ...
      if (
        ...
        navigation.navigate(AuthStacks.ResetVerification, {
          formData: {username: email, email, password, role: flowType},
        });
      }
    } catch (error) {
      showAlert &&
    ...
    if (
      ...
      showAlert &&
        showAlert(
          true,
          'Email Alert',
          'Do we have your permission to send an Email with verification code?',
          [
            {
              label: 'No',
              action: () => {

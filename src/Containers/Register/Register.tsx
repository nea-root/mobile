...
const Register: React.FC = () => {
    ...
    const myFunc = async () => {
        try {
            const response = await signIn('varunrachakatla','070894@Anjali','victim')
            // console.log(response) 
            const response_2 = await getTokens('victim')
        ...
        if (flowType === UserFlowTypes.victim) {
            showAlert && showAlert(true, '', 'Do we have your permission to send an Email with verification code?', [
              ...
              },
              {
                ...
                action: () => {
                  hideModal && hideModal()
                  navigation.navigate(AuthStacks.Verification)
                },
              },
            ])
          }
    ...
    return (
        <View style={{flex: 1}}>
            <AuthForm mode={'register'} onSubmit={handleRegisterClick} buttonLabel={'Register'} showDropdown={true} email={email} username={username} password={password} place={place} handleInput={handleInput}/>
        </View>
    );
};
